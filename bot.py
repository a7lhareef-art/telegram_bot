...
async def start(update: Update, context):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('INSERT OR IGNORE INTO users (id, username, first_name) VALUES (?, ?, ?)', (uid, uname, fname))
    conn.commit()
...
async def button_handler(update: Update, context):
    ...
    if data == "back_to_main":
        ...
        if is_admin(uid):
            keyboard.append([InlineKeyboardButton("⚙️ لوحة الأدمن", callback_data="admin_panel")])
        await query.edit_message_text(f"📋 **القائمة الرئيسية - {SHOP_NAME}**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        return

    if data == "contact":
        await query.edit_message_text(f"📞 **تواصل معنا**\n\nللدعم: {SUPPORT_USERNAME}\n[اضغط هنا]({SUPPORT_LINK})", parse_mode='Markdown', disable_web_page_preview=True)
        return

    if data == "view_cart":
        conn = get_db()
        orders = conn.cursor().execute('''SELECT id, product_name, price, status, created_at FROM orders
...
async def admin_panel(update, context):
    ...
    if not is_admin(q.from_user.id):
        ...
        return
    await q.edit_message_text("⚙️ **لوحة التحكم**", reply_markup=InlineKeyboardMarkup(ADMIN_MENU), parse_mode='Markdown')
...
async def broadcast_start(update, context):
    ...
    if not is_admin(q.from_user.id):
        await q.edit_message_text("⛔ للمدير فقط!")
        return
    await q.edit_message_text("📢 أرسل رسالة الإذاعة:", parse_mode='Markdown')
...
async def broadcast_send(update, context):
    ...
    for u in users:
        try:
            ...
            f += 1
    await update.message.reply_text(f"✅ تم الإرسال لـ {s} مستخدم، ❌ فشل {f}", parse_mode='Markdown')
    return ConversationHandler.END
