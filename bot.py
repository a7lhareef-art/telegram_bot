...
@bot.message_handler(func=lambda message: message.text in TELEGRAM_NUMBERS.keys())
def buy_telegram_number(message):
    user_id = message.from_user.id
    country = message.text
    number = TELEGRAM_NUMBERS[country]
    
    if not can_use_trial(user_id):
        bot.send_message(
            message.chat.id,
            "❌ **انتهت التجارب المجانية!**\n\n"
            "💰 يمكنك شحن الرصيد للاستمرار",
            reply_markup=main_keyboard
        )
        return
