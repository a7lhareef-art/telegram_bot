...
# ====== أرقام تليجرام ======
TELEGRAM_NUMBERS = {
    "🇮🇳 الهند ⚠️ SPAM - 0.25$": "يرجى التواصل مع الدعم",
    "🇺🇸 أمريكا ⚠️ SPAM - 0.30$": "يرجى التواصل مع الدعم",
    "🇲🇲 ميانمار ⚠️ SPAM - 0.30$": "يرجى التواصل مع الدعم",
    "🇲🇦 المغرب - 0.60$": "يرجى التواصل مع الدعم",
}
...
telegram_keyboard.add(
    KeyboardButton("🇮🇳 الهند ⚠️ SPAM - 0.25$")
)
telegram_keyboard.add(
    KeyboardButton("🇺🇸 أمريكا ⚠️ SPAM - 0.30$")
)
telegram_keyboard.add(
    KeyboardButton("🇲🇲 ميانمار ⚠️ SPAM - 0.30$")
)
telegram_keyboard.add(
    KeyboardButton("🇲🇦 المغرب - 0.60$")
)
...
def send_activation_message(user_id, username, first_name, service_type, data, order_id):
    try:
        ...
        if service_type == "بروكسي":
            ...
            msg += (
                f"┃ 🌍 الدولة: {data.get('country', '')}\n"
                f"┃ 📱 الرقم/الحساب: `{data.get('number', '')}`\n"
            )
...
def process_telegram_order(message, country, price):
    user_id = message.from_user.id
    ...
    send_activation_message(
        ...
        "حساب تليجرام",
        {"country": country, "number": "جاري التجهيز"},
        order_id
    )

@bot.message_handler(func=lambda message: message.text == "🇮🇳 الهند ⚠️ SPAM - 0.25$")
def buy_india(message):
    process_telegram_order(message, "🇮🇳 الهند ⚠️ SPAM", 0.25)

@bot.message_handler(func=lambda message: message.text == "🇺🇸 أمريكا ⚠️ SPAM - 0.30$")
def buy_usa(message):
    process_telegram_order(message, "🇺🇸 أمريكا ⚠️ SPAM", 0.30)

@bot.message_handler(func=lambda message: message.text == "🇲🇲 ميانمار ⚠️ SPAM - 0.30$")
def buy_myanmar(message):
    process_telegram_order(message, "🇲🇲 ميانمار ⚠️ SPAM", 0.30)

@bot.message_handler(func=lambda message: message.text == "🇲🇦 المغرب - 0.60$")
def buy_morocco(message):
    process_telegram_order(message, "🇲🇦 المغرب", 0.60)
...
def buy_whatsapp_number(message):
    ...
    number = WHATSAPP_NUMBERS[country]
...
def get_proxy_password(message):
    ...
    price = PROXY_PACKAGES.get(data.get("duration", "1 day"), 93)
